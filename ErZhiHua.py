...
class GUI:
    ...
    def read_show_save_img(file_pathname, outfile):
        ...
        for filename in os.listdir(file_pathname):
            if filename.endswith('.png'):
                ...
                upper =BGR+15
                lower =BGR-15
                #OpenCV提供了cv2.inRange函数，这个函数对图片进行二值化处理，
                # 将不在阀值范围内的点变换为黑色点[0,0,0]，处于范围内的点变换为白色点[255,255,255]。
                ...
                cv2.imshow('image',mask)  # 窗口会自动调整为图像大小。第一个参数是窗口的名字
                k = cv2.waitKey(0)
                ...
                if k == 27:  # esc键
                    ...
                    # 【3】保存图像
                    cv2.imwrite(outfile+"/"+filename,mask)
                    cv2.destroyAllWindows()  # 清除所有窗口
    ...
    read_show_save_img("./images",'./erzhihua')
